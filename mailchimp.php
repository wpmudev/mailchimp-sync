...
function mailchimp_add_wpmu_user($uid) {
	global $wpdb;
	$user_email = $wpdb->get_var("SELECT user_email FROM " . $wpdb->users . " WHERE ID = '" . $uid . "'");
	if ( get_site_option('mailchimp_ignore_plus') == 'yes' && strstr($user_email, '+') ) {
		unset($user_email);
	}
	if ( !empty( $user_email ) ) {
		...
		$mailchimp_api = new MCAPI($mailchimp_username, $mailchimp_password);
		...
		$mailchim_subscribe = $mailchimp_api->listSubscribe($mailchimp_mailing_list, $user_email, $merge_vars, '', $double_optin);
	}
}

function mailchimp_add_wp_user($uid) {
	global $wpdb;
	$user_email = $wpdb->get_var("SELECT user_email FROM " . $wpdb->users . " WHERE ID = '" . $uid . "'");
	if ( get_option('mailchimp_ignore_plus') == 'yes' && strstr($user_email, '+') ) {
		unset($user_email);
	}
	if ( !empty( $user_email ) ) {
		$mailchimp_username = get_option('mailchimp_username');
		$mailchimp_password = get_option('mailchimp_password');
		$mailchimp_mailing_list = get_option('mailchimp_mailing_list');
		$mailchimp_auto_opt_in = get_option('mailchimp_auto_opt_in');
		$mailchimp_api = new MCAPI($mailchimp_username, $mailchimp_password);
		if ( $mailchimp_auto_opt_in == 'yes' ) {
			$merge_vars = array( 'OPTINIP' => $_SERVER['REMOTE_ADDR'] );
			$double_optin = false;
	...
	switch( $_GET[ 'action' ] ) {
		...
		default:
			if ( function_exists("is_site_admin") ) {
				...
				$mailchimp_mailing_list = get_option('mailchimp_mailing_list');
				$mailchimp_auto_opt_in = get_option('mailchimp_auto_opt_in');
				$mailchimp_ignore_plus = get_option('mailchimp_ignore_plus');
			}
			?>
			...
			if ( empty( $mailchimp_username ) && empty( $mailchimp_password ) ) {
			?>
            <p><?php _e('After you have filled out your account details you will be able to select different MailChimp options below.'); ?></p>
            <?php
			}
			?>
            <table class="form-table">
                <tr class="form-field form-required">
                    ...
                    <td><input type="text" name="mailchimp_username" id="mailchimp_username" value="<?php echo $mailchimp_username; ?>" style="width:25%" /><br />
                    <?php _e('Your MailChimp Username'); ?>
                    </td>
                </tr>
                <tr class="form-field form-required">
                    <th scope="row"><?php _e('Password')?></th>
                    <td><input type="password" name="mailchimp_password" id="mailchimp_password" value="<?php echo $mailchimp_password; ?>" style="width:25%" /><br />
                    <?php _e('Your MailChimp Password'); ?>
                    </td>
                </tr>
                <?php
                if ( !empty( $mailchimp_username ) && !empty( $mailchimp_password ) ) {
					$mailchimp_api = new MCAPI($mailchimp_username, $mailchimp_password);
					if ( empty( $mailchimp_api->errorMessage ) ) {
						?>
						<tr class="form-field form-required">
							<th scope="row"><?php _e('Login Test')?></th>
							<td><strong style="color:#006633"><?php _e('Passed')?></strong>
							</td>
						</tr>
                        <tr class="form-field form-required">
                            <th scope="row"><?php _e('Auto Opt-in')?></th>
                            <td><select name="mailchimp_auto_opt_in" id="mailchimp_auto_opt_in">
                                <option value="yes" <?php if ( $mailchimp_auto_opt_in == 'yes' ) { echo 'selected="selected"'; } ?> ><?php _e('Yes'); ?></option>
                                <option value="no" <?php if ( $mailchimp_auto_opt_in == 'no' ) { echo 'selected="selected"'; } ?> ><?php _e('No'); ?></option>
                            </select>
                            <br />
                            <?php _e('Automatically optin-in new users to the mailing list. Users will not receive an email confirmation. Use at your own risk.'); ?>
                            </td>
                        </tr>
                        <tr class="form-field form-required">
                            <th scope="row"><?php _e('Ignore address including + signs')?></th>
                            <td><select name="mailchimp_ignore_plus" id="mailchimp_ignore_plus">
                                <option value="yes" <?php if ( $mailchimp_ignore_plus == 'yes' ) { echo 'selected="selected"'; } ?> ><?php _e('Yes'); ?></option>
                                <option value="no" <?php if ( $mailchimp_ignore_plus == 'no' ) { echo 'selected="selected"'; } ?> ><?php _e('No'); ?></option>
                            </select>
                            <br />
                            <?php _e('Ignore email address including + signs. These are usually duplicate accounts.'); ?>
                            </td>
                        </tr>
						<?php
						$mailchimp_lists = $mailchimp_api->lists();
						if ( count( $mailchimp_lists ) < 1 || !is_array( $mailchimp_lists ) ) {
							_e('You must have at least one MailChimp mailing list in order to use this plugin. Please creat a mailing list via the MailChimp admin panel.');
						} else {
							?>
                            <tr class="form-field form-required">
                                <th scope="row"><?php _e('Mailing List')?></th>
                                <td><select name="mailchimp_mailing_list" id="mailchimp_mailing_list">
                                <?php
								if ( empty( $mailchimp_mailing_list ) ) {
									?>
									<option value="" selected="selected" ><?php _e('Please select a mailing list'); ?></option>
                                    <?php
								}
                            	...
                            	<br />
                                <?php _e('The mailing list you want to add new users to.'); ?>
                                </td>
                            </tr>
							<?php
						}
						...
						<tr class="form-field form-required">
							<th scope="row"><?php _e('Login Test')?></th>
							<td><strong style="color:#990000"><?php _e('Failed')?></strong>
							</td>
						</tr>
						<?php
					}
			...
			<?php
            if ( count( $mailchimp_lists ) > 0 || is_array( $mailchimp_lists ) ) {
				?>
				...
				if ( function_exists("is_site_admin") ) {
					...
					if ( get_option('mailchimp_imported') == 'yes' ) {
						?>
                        <p><?php _e('You have already imported existing users once. Please be careful not to import users into the same list twice.'); ?></p>
                        <?php
					}
				...
				<table class="form-table">
                    <tr class="form-field form-required">
                        ...
                        <br />
                        <?php _e('The mailing list you want to import existing users to.'); ?>
                        </td>
                    </tr>
                    <tr class="form-field form-required">
                        <th scope="row"><?php _e('Auto Opt-in')?></th>
                        ...
                        <br />
                        <?php _e('Automatically optin-in existing users to the mailing list. Users will not receive an email confirmation. Use at your own risk.'); ?>
                        </td>
                    </tr>
				</table>
				
				<p class="submit">
				<input type="submit" name="Submit" value="<?php _e('Import') ?>" />
				</p>
				</form>
				<?php
            }
		...
		case "process":
			if ( function_exists("is_site_admin") ) {
				...
				update_option('mailchimp_username', $_POST['mailchimp_username']);
				update_option('mailchimp_password', $_POST['mailchimp_password']);
				update_option('mailchimp_mailing_list', $_POST['mailchimp_mailing_list']);
			...
			if ( function_exists("is_site_admin") ) {
				...
				echo "
				<SCRIPT LANGUAGE='JavaScript'>
				window.location='options-general.php?page=mailchimp&updated=true&updatedmsg=" . urlencode(__('Settings saved.')) . "';
				</script>
				";
			}
		break;
		...
		case "import-process":
			?>
			<h2><?php _e('Importing Existing Users') ?></h2>
            <p>...</p>
            <?php
				...
				if ( count( $existing_users ) > 0 ) {
					foreach ( $existing_users as $existing_user ) {
						if ( $mailchimp_ignore_plus == 'yes' ) {
							if ( strstr($existing_user['user_email'], '+') ) {
								$continue = 'no';
							} else {
								$continue = 'yes';
							}
						...
						if ( $continue == 'yes' ) {
							...
							$mailchim_subscribe = $mailchimp_api->listSubscribe($mailchimp_import_mailing_list, $existing_user['user_email'], $merge_vars, '', $double_optin);
						}
					}
				}
			}
			if ( function_exists("is_site_admin") ) {
				...
				echo "
				<SCRIPT LANGUAGE='JavaScript'>
				window.location='ms-admin.php?page=mailchimp&updated=true&updatedmsg=" . urlencode(__('Existing users imported.')) . "';
				</script>
				";
			} else {
				update_option('mailchimp_imported', 'yes');
				echo "
				<SCRIPT LANGUAGE='JavaScript'>
				window.location='options-general.php?page=mailchimp&updated=true&updatedmsg=" . urlencode(__('Existing users imported.')) . "';
				</script>
				";
			}
		break;
